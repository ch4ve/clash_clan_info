        ...
        try:
            ...
            async for war in group.get_wars(cwl_round=coc.WarRound.current_war):
                if war.clan.tag == clan_tag or war.opponent.tag == clan_tag:
                    ...
                    clan_members_data = [{'Pos.': member.map_position, 'Nome': member.name, 'CV': member.town_hall} for member in clan_side.members]
                    df_clan = pd.DataFrame(clan_members_data).sort_values(by='Pos.')
                    opponent_members_data = [{'Pos.': member.map_position, 'Nome': member.name, 'CV': member.town_hall} for member in opponent_side.members]
...
def get_cwl_schedule(clan_tag, coc_email, coc_password):
    ...
    async def _fetch_schedule():
        ...
        try:
            ...
            schedule = []
            for war_day, round_tags_list in enumerate(group.rounds, 1): # round_tags_list é a lista de tags do dia
                
                # --- CORREÇÃO APLICADA AQUI ---
                # Iteramos diretamente sobre a lista de tags (round_tags_list)
